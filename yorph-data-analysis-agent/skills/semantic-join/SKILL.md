...
Sample the dataset(s) and identify what structured information is latent in the text fields. Use an LLM call on the sample (not the full dataset) to propose a set of extraction columns that would be useful given the user's goal.
...
- **LLM batch call**: for fields that genuinely need semantic understanding (e.g. inferring category from a product description) — batch rows together to minimize calls
