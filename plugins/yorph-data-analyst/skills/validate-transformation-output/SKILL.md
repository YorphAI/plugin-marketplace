...
- Waterfall → `validate_waterfall()` from the `waterfall-chart` skill (closure check: start + deltas = end)
- Cohort → `validate_cohort_table()` from the `cohort-heatmap-chart` skill (period 0 = 100%, no negatives, no missing p0 cohorts)
...
Apply domain common sense to aggregate statistics. Reference the `design-transformation-architecture` skill's `references/domain-rules.md` for domain-specific constraints.
