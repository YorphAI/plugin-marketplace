---
name: orchestrate-data-analysis
description: Use this skill any time the user wants to work with data. Triggers include "analyze my data", "look at this data", "what's in my data", "upload a file", "connect to my database", "I have a CSV", "I have a spreadsheet", "help me understand this dataset", "find insights", "build a dashboard", "show me trends", "why did revenue drop", "compare these segments", "clean my data", "transform this data", "run an analysis", "what does this data tell me", "explore this dataset", "give me a report", "summarize my data", "data analysis", "data engineering", "ETL", "create a pipeline", "visualize my data", "chart this", "plot this", "what happened last quarter", "break down by region", "cohort analysis", "retention analysis", "funnel analysis", "attribution analysis", "variance analysis", "root cause analysis", "waterfall chart", "tell me the story in this data", or any request involving connecting to, exploring, transforming, analyzing, or visualizing structured data.
---
...
**Architecture skill** (`yorph-data-analyst:design-transformation-architecture`) — Load this via the Skill tool before designing any transformation steps. It contains the domain rules, data-cleaning patterns, and methodology templates that turn a vague user goal into a sound plan. Without it, plans tend to miss edge cases (null handling, grain mismatches, ambiguous column semantics) that blow up downstream.

**Pipeline-builder agent** (`yorph-data-analyst:pipeline-builder`) — Spawn this via the Agent tool for all data transformation work. The agent has sampling, validation, and scale-execution skills baked in — it handles the full build-validate-scale cycle autonomously. Writing transformation code yourself bypasses validation and produces unverified results.
...
Load the `connect-data-source` skill and the `profile-data` skill.
...
Load the `design-transformation-architecture` skill (`yorph-data-analyst:design-transformation-architecture`) via the Skill tool. It contains domain rules, cleaning patterns, and methodology templates that catch edge cases you'd otherwise miss. Use it to design the transformation logic as a set of named, ordered, plain-English steps — no code yet. Each step should be understandable to a non-technical user.

Get explicit user approval on the architecture before handing off to execution.

### Step 4 — Execute pipeline

Spawn the `pipeline-builder` agent (`yorph-data-analyst:pipeline-builder`) via the Agent tool. It handles sampling, code generation, validation, and full-scale execution autonomously — that's why the code lives there, not here.
...
1. **`derive-insights` skill** → study the output, formulate deeper analytical questions, delegate them back to the pipeline-builder agent in 1–3 rounds, then synthesize into ≤5 ranked headline findings.
